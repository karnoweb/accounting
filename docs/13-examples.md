...
`activate()` روی سال مالی، اگر `period.auto_create_on_activate` روشن باشد (پیش‌فرض)، یک دورهٔ `open` تمام‌سال می‌سازد. هم `save()` و هم `post()` به سال `active` و دورهٔ `open` نیاز دارند.
